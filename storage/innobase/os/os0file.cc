 ...
 byte *os_file_compress_page(Compression compression, ulint block_size,
   ...
   switch (compression.m_type) {
     ...
     case Compression::ZLIB: {
       ...
       std::ofstream outfile("/home/mysql/output.txt", std::ios::app); // 以追加模式打开文件
       if (!outfile) {
           std::cerr << "无法打开文件 /home/mysql/output.txt" << std::endl;
           outfile.close();
       }
       
       outfile << "zlib compression metrics: original=" << content_len
               << ", compressed=" << len
               << ", ratio=" << std::fixed << std::setprecision(5) << static_cast<double>(len) / content_len
               << ", time=" << duration_ms.count() << " mcs"
               << std::endl;
 ...
 
     {
       ...
       outfile << "lz4 compression metrics: original=" << content_len
               << ", compressed=" << len
               << ", ratio=" << std::fixed << std::setprecision(5) << static_cast<double>(len) / content_len
               << ", time=" << duration_ms.count() << " mcs"
               << std::endl;
     ...
     {
       ulint zstd_compression_level = 3;
 
       auto start = high_resolution_clock::now();
       len =
           ZSTD_compress(reinterpret_cast<char *>(dst) + FIL_PAGE_DATA,
                         static_cast<int>(out_len),
                         reinterpret_cast<char *>(src) + FIL_PAGE_DATA,
                         static_cast<int>(content_len), zstd_compression_level);
       auto end = high_resolution_clock::now();
       auto duration_ms = duration_cast<microseconds>(end - start);
       ...
       outfile << "zstd compression metrics: original=" << content_len
               << ", compressed=" << len
               << ", ratio=" << std::fixed << std::setprecision(5) << static_cast<double>(len) / content_len
               << ", time=" << duration_ms.count() << " mcs"
               << std::endl;
       ...
       if (ZSTD_isError(len) || len == 0 || len >= out_len) {
         *dst_len = src_len;
